            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-admin-border/50">
                    ...
                    <tr class="hover:bg-admin-card-hover/50 transition-colors">
                        ...
                        <td class="px-6 py-4 font-medium">{{ $p->nama_penerima }}</td>
                        <td class="px-6 py-4 text-text-muted max-w-[200px] truncate" title="{{ $p->alamat_pengiriman }}">{{ $p->alamat_pengiriman }}</td>
                        <td class="px-6 py-4">{{ $p->kurir ?? '-' }}</td>
                        <td class="px-6 py-4 font-mono text-xs">{{ $p->no_resi ?? '-' }}</td>
                        <td class="px-6 py-4">
                            @php
                            ...
                            @endphp
                            <span class="px-2.5 py-1 rounded-full text-xs font-medium {{ $colors[$p->status_pengiriman] ?? '' }}">{{ ucfirst($p->status_pengiriman) }}</span>
                        </td>
                        <td class="px-6 py-4 text-text-muted">{{ $p->tanggal_kirim ?? '-' }}</td>
                        <td class="px-6 py-4">
                            ...
                            <div id="ship-modal-{{ $p->id }}" class="hidden fixed inset-0 bg-black/60 z-50 flex items-center justify-center" onclick="if(event.target===this)this.classList.add('hidden')">
                                <div class="bg-admin-card border border-admin-border rounded-2xl p-6 w-[450px]">
                                    <h4 class="font-semibold mb-4">Update Pengiriman #{{ $p->id }}</h4>
                                    <form method="POST" action="{{ route('admin.shipping.updateStatus', $p) }}">
                                        @csrf @method('PATCH')
                                        <div class="space-y-3">
                                            <div>
                                                <label class="text-xs text-text-muted">Status</label>
                                                <select name="status_pengiriman" class="w-full bg-admin-bg border border-admin-border rounded-xl px-4 py-2.5 text-sm text-text-primary">
                                                    @foreach(['pending','dikirim','sampai','dibatalkan'] as $s)
                                            ...
                                            <div class="grid grid-cols-2 gap-3">
                                                <div>
                                                    ...
                                                    <input type="text" name="kurir" value="{{ $p->kurir }}" class="w-full bg-admin-bg border border-admin-border rounded-xl px-4 py-2.5 text-sm text-text-primary">
                                                </div>
                                                <div>
                                                    <label class="text-xs text-text-muted">No Resi</label>
                                                    <input type="text" name="no_resi" value="{{ $p->no_resi }}" class="w-full bg-admin-bg border border-admin-border rounded-xl px-4 py-2.5 text-sm text-text-primary">
                                                </div>
                                            </div>
                                            <div class="grid grid-cols-2 gap-3">
                                                <div>
                                                    <label class="text-xs text-text-muted">Tgl Kirim</label>
                                                    <input type="date" name="tanggal_kirim" value="{{ $p->tanggal_kirim }}" class="w-full bg-admin-bg border border-admin-border rounded-xl px-4 py-2.5 text-sm text-text-primary">
                                                </div>
                                                <div>
                                                    <label class="text-xs text-text-muted">Tgl Terima</label>
                                                    <input type="date" name="tanggal_terima" value="{{ $p->tanggal_terima }}" class="w-full bg-admin-bg border border-admin-border rounded-xl px-4 py-2.5 text-sm text-text-primary">
                                                </div>
                                            </div>
                                        </div>
                                        <div class="flex gap-2 mt-4">
                                            ...
                                            <button type="button" onclick="this.closest('[id^=ship-modal]').classList.add('hidden')" class="flex-1 bg-admin-bg border border-admin-border py-2.5 rounded-xl text-sm">Batal</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
