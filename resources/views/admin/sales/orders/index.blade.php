        ...
        <form method="GET" class="flex flex-wrap gap-4 items-end">
            ...
            <div class="w-48">
                ...
                <select name="status" class="w-full bg-admin-bg border border-admin-border rounded-xl px-4 py-2.5 text-sm text-text-primary focus:border-accent-emerald outline-none">
                    ...
                    @foreach(['pending','paid','success','shipped','completed','cancelled','failed','expired'] as $s)
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-admin-border/50">
                    ...
                    <tr class="hover:bg-admin-card-hover/50 transition-colors">
                        ...
                        <td class="px-6 py-4 text-text-muted">{{ $order->customer_email ?? '-' }}</td>
                        <td class="px-6 py-4 font-medium text-accent-gold">Rp {{ number_format($order->total_harga, 0, ',', '.') }}</td>
                        <td class="px-6 py-4">
                            @php
                                $colors = ['pending'=>'bg-yellow-500/10 text-yellow-400','paid'=>'bg-blue-500/10 text-blue-400','shipped'=>'bg-purple-500/10 text-purple-400','completed'=>'bg-emerald-500/10 text-emerald-400','cancelled'=>'bg-red-500/10 text-red-400','failed'=>'bg-red-500/10 text-red-400','success'=>'bg-emerald-500/10 text-emerald-400'];
                        ...
                        <td class="px-6 py-4 text-xs text-text-muted">{{ $order->metode_pembayaran ?? '-' }}</td>
                        <td class="px-6 py-4 text-text-muted">{{ $order->created_at->format('d M Y') }}</td>
                        <td class="px-6 py-4">
                            ...
                            <div id="status-modal-{{ $order->id }}" class="hidden fixed inset-0 bg-black/60 z-50 flex items-center justify-center" onclick="if(event.target===this)this.classList.add('hidden')">
                                <div class="bg-admin-card border border-admin-border rounded-2xl p-6 w-96">
                                    <h4 class="font-semibold mb-4">Update Status Order #{{ $order->id }}</h4>
                                    <form method="POST" action="{{ route('admin.orders.updateStatus', $order) }}">
                                        @csrf @method('PATCH')
                                        <select name="status" class="w-full bg-admin-bg border border-admin-border rounded-xl px-4 py-2.5 text-sm mb-4 text-text-primary">
                                            @foreach(['pending','paid','success','failed','expired','cancelled','shipped','completed'] as $s)
                                        ...
                                        <div class="flex gap-2">
                                            ...
                                            <button type="button" onclick="this.closest('[id^=status-modal]').classList.add('hidden')" class="flex-1 bg-admin-bg border border-admin-border py-2.5 rounded-xl text-sm">Batal</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
